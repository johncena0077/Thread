...
class ViewController1: UIViewController {
    ...
    func tuple(){
        ...
        let tp = (name :"tuple return multiple value types", cal:12)
        
        print(tp.name)
        
        switch tp {
        case let (t1,t2):
            print("tuple return multiple value types \(t1) return multiple value types \(t2)")
        default:
            print("")
        }
    ...
    func testCloseureEscaping() {
        //setp 1
        self.getSumOf(array: [1,1,1]) { [weak self](sum) in
            print("testCloseureEscaping  \(sum)")
            //step 4, finishing the execution
            
        }
       // print("11asdf")
        sleep(4)
        print("testCloseureEscaping\(self.comple!(2))")
